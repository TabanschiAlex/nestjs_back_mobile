...
export class ProductController implements BasicController {
  ...
  public async index(@Query() query: BasicQueryRequest, @Res() res: Response) {
    try {
      return res
        .send(await this.productService.getAll(query))
  ...
  public async edit(@Param('id') id: string, @Res() res: Response) {
    try {
      return res
        .send(await this.productService.getOne(id))
        .status(HttpStatus.OK);
    } catch (e) {
      res.send(e.getResponse()).status(e.getStatus());
    }
  ...
  public async store(
    ...
    try {
      ...
      return res
        .send('New manufacturer added successfully')
  ...
  public async update(
    ...
    try {
      ...
      return res
        .send('Manufacturer updated successfully')
  ...
  public async destroy(@Param('id') id: string, @Res() res: Response) {
    try {
      ...
      return res
        .send('Manufacturer deleted successfully')
        .status(HttpStatus.NO_CONTENT);
    } catch (e) {
      res.send(e.getResponse()).status(e.getStatus());
    }
